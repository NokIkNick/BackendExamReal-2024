    ...
    public Handler register() {
        return (ctx) -> {
            ...
            try{
                ...
                ctx.status(HttpStatus.CREATED).json(new TokenDTO(token, userInput.getUsername())).header("Access-Control-Allow-Origin","*");
    ...
    public Handler login(){
        return (ctx) -> {
            ...
            try{
                ...
                ctx.status(200).json(new TokenDTO(token, user.getUsername())).header("Access-Control-Allow-Origin","*");
