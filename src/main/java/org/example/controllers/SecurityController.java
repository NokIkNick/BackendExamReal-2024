    ...
    public Handler register() {
        return (ctx) -> {
            ...
            try{
                ...
                ctx.status(HttpStatus.CREATED).json(new TokenDTO(token, userInput.getUsername(), created.getRolesToString().stream().toList().toString()));
    ...
    public Handler login(){
        return (ctx) -> {
            ...
            try{
                ...
                ctx.status(200).json(new TokenDTO(token, user.getUsername(), verifiedUserEntity.getRolesToString().stream().toList().toString()));
